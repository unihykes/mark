Linux命令：netstat：显示网络信息/检查服务运行状态

```
netstat –apn  查看端口检查服务是否启动成功：
netstat -anp | grep efast

查看网络连接
netstat -anp |grep xxx
     Local Address 本地端的地址及端口
     Foreign Address 远程的主机IP及端口
     stat：状态
     ESTABLISED 已建立连接的状态ping
     TIME_WAIT:该连接已经挂断，但socket还在网络上等待结束
...
tcp 0 0 ramesh-laptop.loc:52750 lax:www ESTABLISHED 2109/firefox5. 在 netstat 输出中不显示主机，端口和用户名 (host, port or user)
...
注意： 使用 netstat -rn 显示数字格式，不查询主机名称。9. 找出程序运行的端口
...
# netstat -an | grep ':80'10. 显示网络接口列表
