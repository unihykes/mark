...
TEST(ut_ReferenceCollapsing, typedef)
{
    ...
    MK_PRINT_MSG("lRef&              is %c-value Ref", std::is_lvalue_reference<lRef&>::value? 'L':'R');
     
    lRef&& r2 = n; // r2 的类型是 int&, 到左值引用类型(lRef)的右值引用&&,折叠为左值引用
    MK_PRINT_MSG("lRef&&             is %c-value Ref", std::is_lvalue_reference<lRef&&>::value? 'L':'R');
    
    rRef&  r3 = n; // r3 的类型是 int&, 到右值引用类型(rRef)的左值引用&,折叠为左值引用
    MK_PRINT_MSG("rRef&              is %c-value Ref", std::is_lvalue_reference<rRef&>::value? 'L':'R');
...
TEST(ut_ReferenceCollapsing, template3)
{
    ...
    //nL是左值引用,此时T的类型是int&, 则const T&&表示到左值引用类型的右值引用, const [int&]&& 折叠为const int&, 按道理不应该编译错误?为何??????? 
    fun_collapsing_c<int&>(nL);
    
    
    //编译错误: nR是右值引用,此时T的类型是int&&, 则const T&&表示到右值引用类型的右值引用, const [int&&]&& 折叠为const int&&, nR是左值,无法绑定到const右值引用上,编译错误; 
