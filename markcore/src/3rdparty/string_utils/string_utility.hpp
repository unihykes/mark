#ifndef __string_utility
#define __string_utility
...
public:
    ...
    static bool starts_with_ignore_case(Str const & value, Str const & starting)
    {
        if (starting.size() > value.size()) return false;
    ...
    static bool ends_with_ignore_case(Str const & value, Str const & ending)
    {
        if (ending.size() > value.size()) return false;
    ...
    {
        std::vector<Str> ss;
        tokenizer token(str, delimiters);
